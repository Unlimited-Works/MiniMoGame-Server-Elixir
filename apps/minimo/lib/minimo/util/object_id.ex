defmodule Minimo.Util.ObjectId do
  @moduledoc """
  ...
  def new(machine_id, proc_id, secs, counter) do
    value = <<secs       :: unsigned-big-32,#4
              machine_id :: unsigned-big-24,#3
              proc_id    :: unsigned-big-16,#2
              counter    :: unsigned-big-24>>#3
    %Minimo.Util.ObjectId{value: value}
  end
  ...
  """
  def encode!(%Minimo.Util.ObjectId{value: value}), do: do_encode(value)
  ...
  defimpl Inspect do
    def inspect(objectid, _opts) do
      encoded = Minimo.Util.ObjectId.encode!(objectid)
